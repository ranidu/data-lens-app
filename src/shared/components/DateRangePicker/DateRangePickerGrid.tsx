...
const DateRangePickerGrid = ({
  ...
  const cellProps = useMemo(() => {
    ...
    const isRangeExceed = rangeLimit
      ? isSelecting &&
        hoverDate !== null &&
        startDate !== null &&
    ...
    return days.map((date) => {
      ...
      const isRangeStart = !!(startDate && date.isSame(startDate, "day")); //style
      const isRangeEnd = !!(
        rangeEnd &&
        date.isSame(rangeEnd, "day") &&
        !isSelecting
      ); //style
      const inRange = isInRange(date, rangeStart, rangeEnd); //style
      const inHoverRange = isSelecting && isInRange(date, rangeStart, rangeEnd);
      const inExceededRange =
        isRangeExceed && startDate !== null && hoverDate !== null;
      ...
      return {
        ...
        isHoverRange:
          isSelecting && !isRangeExceed && (inHoverRange || isRangeStart),
  ...
  const handleClick = useCallback(
    (date: Dayjs) => onDateClick(date),
    [onDateClick],
  );
  const handleHover = useCallback(
    (date: Dayjs) => onDateHover(date),
    [onDateHover],
  );
    ...
    <div>
      <div className="grid grid-cols-7 mb-1">
        {DAY_LABELS.map((label) => (
          <div
            key={label}
            className="flex items-center justify-center h-8 text-xs font-medium text-gray-400 select-none"
          >
            {label}
          </div>
        ))}
